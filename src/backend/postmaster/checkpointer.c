 ...
 */
void CheckpointerMain(void) // 检查点进程的入口函数
{
  sigjmp_buf local_sigjmp_buf;
  MemoryContext checkpointer_context;

  CheckpointerShmem->checkpointer_pid =
      MyProcPid; /// 记录一下检查点进程的进程号
  ...
  for (;;) {
    ...
    elapsed_secs =
        now - last_checkpoint_time; // 把当前时间减去上一次检查点的时间，
    if (elapsed_secs >=
        CheckPointTimeout) // 如果超过了检查点的超时时间，就必须要做检查点操作了
    {
      if (!do_checkpoint)
        PendingCheckpointerStats
            .timed_checkpoints++; // 因为超过了时间而导致的检查点
      do_checkpoint = true;
    ...
    if (do_checkpoint) // 如果要执行检查点了
    {
      ...
      do_restartpoint =
          RecoveryInProgress(); // 在备库上做restartpoint，而不是检查点
      ...
      ckpt_active = true;
      if (do_restartpoint)
        ckpt_start_recptr = GetXLogReplayRecPtr(
            NULL); // 就是读取XLogRecoveryCtl->lastReplayedEndRecPtr
      else
     ...
     */
    if (((volatile CheckpointerShmemStruct *)CheckpointerShmem)
            ->ckpt_flags) // 如果此时还有新的检查点请求，就返回起点执行检查点
      continue;
    ...
    if (elapsed_secs >=
        CheckPointTimeout) // 又超时了，继续返回起点开始执行检查点
 ...
 * records.
 */
static void CheckArchiveTimeout(void) // 检查归档超时的问题
{
  pg_time_t now;
  pg_time_t last_time;
  ...
  if ((int)(now - last_xlog_switch_time) <
      XLogArchiveTimeout) // XLogArchiveTimeout记录超时的时间值
    return;
  ...
  last_xlog_switch_time =
      Max(last_xlog_switch_time, last_time); // 取两者的最大值

  /* Now we can do the real checks */
  if ((int)(now - last_xlog_switch_time) >=
      XLogArchiveTimeout) // 超时了，开始做切换
  {
    /*
 ...
 */
static bool ImmediateCheckpointRequested(
    void) // 在共享内存中检查状态为是否为CHECKPOINT_IMMEDIATE
{
...
void CheckpointWriteDelay(int flags, double progress) {
   ...
   */
  if (!(flags & CHECKPOINT_IMMEDIATE) && !ShutdownRequestPending &&
     ...
     * Sleep.
     */
    WaitLatch(MyLatch, WL_LATCH_SET | WL_EXIT_ON_PM_DEATH | WL_TIMEOUT, 100,
...
void CheckpointerShmemInit(void) {
  ...
  CheckpointerShmem = (CheckpointerShmemStruct *)ShmemInitStruct(
      "Checkpointer Data", size, &found);
...
void RequestCheckpoint(
    int flags) // 别的子进程调用这个函数，请求执行一个检查点操作
{
  int ntries;
  ...
  CheckpointerShmem->ckpt_flags |=
      (flags | CHECKPOINT_REQUESTED); // 设置一下共享内存的标志位
...
#define MAX_SIGNAL_TRIES 600 /* max wait 60.0 sec */
  for (ntries = 0;; ntries++) {
    if (CheckpointerShmem->checkpointer_pid ==
        0) // 这个时候检查点进程还没有启动
    {
    ...
    } else if (kill(CheckpointerShmem->checkpointer_pid, SIGINT) !=
               0) // 向检查点进程发送一个信号
    {
      if (ntries >= MAX_SIGNAL_TRIES || !(flags & CHECKPOINT_WAIT)) {
        elog((flags & CHECKPOINT_WAIT) ? ERROR : LOG,
...
bool ForwardSyncRequest(const FileTag *ftag, SyncRequestType type) {
  ...
  too_full =
      (CheckpointerShmem->num_requests >= CheckpointerShmem->max_requests / 2);
...
static bool CompactCheckpointerRequestQueue(void) {
  ...
  htab = hash_create("CompactCheckpointerRequestQueue",
                     CheckpointerShmem->num_requests, &ctl,
